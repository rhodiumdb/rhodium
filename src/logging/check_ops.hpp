...
#ifndef RDSS_LOGGING_CHECK_OPS_H_
#define RDSS_LOGGING_CHECK_OPS_H_
...
#define RDSS_LOGGING_INTERNAL_CHECK_OP(name, op, val1, val2)                   \
  while (std::string* rdss_logging_internal_check_op_result                    \
             ABSL_ATTRIBUTE_UNUSED = ::rdss::logging_internal::name##Impl(     \
                 ...
                 RDSS_LOGGING_INTERNAL_STRIP_STRING_LITERAL(#val1 " " #op      \
                                                                  " " #val2))) \
...
#define RDSS_LOGGING_INTERNAL_QCHECK_OP(name, op, val1, val2)                  \
  while (std::string* rdss_logging_internal_qcheck_op_result =                 \
             ::rdss::logging_internal::name##Impl(                             \
                 ...
                 RDSS_LOGGING_INTERNAL_STRIP_STRING_LITERAL(#val1 " " #op      \
                                                                  " " #val2))) \
  RDSS_LOGGING_INTERNAL_QCHECK(*rdss_logging_internal_qcheck_op_result).stream()
...
// Helper functions for `RDSS_LOGGING_INTERNAL_CHECK_OP` macro family. The
...
RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL(Check_NE, !=)
RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL(Check_LE, <=)
RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL(Check_LT, <)
RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL(Check_GE, >=)
RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL(Check_GT, >)
#undef RDSS_LOGGING_INTERNAL_CHECK_OP_IMPL

// `RDSS_CHECK_EQ` and friends want to pass their arguments by reference,
// however this winds up exposing lots of cases where people have defined
// and initialized static const data members but never declared them
// (i.e.: in a `.cc` file), meaning they are not referenceable.
//
