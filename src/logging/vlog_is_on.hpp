...
// Defines the `RDSS_VLOG_IS_ON` macro that controls the variable-verbosity
// conditional logging.
//
// It's used by `RDSS_VLOG` and `RDSS_VLOG_IF` in `logging.h` to trigger
// the logging.
//
// It can also be used directly like this:
//
//   if (RDSS_VLOG_IS_ON(2)) {
//     // do some logging preparation and logging
//     // that can't be accomplished e.g. via just RDSS_VLOG(2) << ...;
//   }
//
// The truth value that `RDSS_VLOG_IS_ON(level)` returns is determined by
...
#ifndef RDSS_LOGGING_VLOG_IS_ON_H_
#define RDSS_LOGGING_VLOG_IS_ON_H_
...
// int32_t at every RDSS_VLOG_IS_ON() call site. The level determines
...
// Set RDSS_VLOG(_IS_ON) level for module_pattern to log_level.
