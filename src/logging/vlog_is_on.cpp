...
std::atomic<int32_t> rdss::logging_internal::vlog_epoch{1};
...
int SetVLOGLevel(absl::string_view module_pattern, int log_level) {
  ...
  for (const VModuleInfo* info = vmodule_list.load(std::memory_order_relaxed);
    ...
    } else if (!found && rdss::logging_internal::SafeFNMatch(
  ...
  ::rdss::logging_internal::vlog_epoch.fetch_add(1, std::memory_order_release);
...
int InitVLOG(std::atomic<int32_t>* site, absl::string_view full_path) {
  ...
  while (info != nullptr) {
    if (info->module_is_path) {
      ...
      if (rdss::logging_internal::SafeFNMatch(info->module_pattern, stem)) {
  ...
  // Attempt to store the new log site. This can race with other threads.
  // If we lose the race, don't bother retrying. Our epoch will remain stale,
  // so we will try again on the next iteration.
