...
void PrepareAndExecInChildProcess(const std::vector<const char*>& argv_pointers,
  ...
  if (!cwd.empty()) {
    if (chdir(cwd.c_str()) != 0) {
      RDSS_LOG(ERROR) << "chdir failed: " << Strerror(errno);
      _exit(127);
    }
  }
  ...
  RDSS_LOG(ERROR) << "Execv syscall failed: " << Strerror(errno);
  _exit(127);
}
...
absl::StatusOr<std::vector<std::string>> ReadFileDescriptors(
  ...
  while (descriptors_left > 0) {
    ...
    for (int i = 0; i < fds.size(); i++) {
      ...
      RDSS_CHECK(!(poll_list[i].revents & POLLNVAL));
...
absl::StatusOr<std::pair<std::string, std::string>> InvokeSubprocess(
  ...
  ASSIGN_OR_RETURN(auto stdout_pipe, Pipe::Open());
  ASSIGN_OR_RETURN(auto stderr_pipe, Pipe::Open());
  ...
  ASSIGN_OR_RETURN(auto output_strings, ReadFileDescriptors(fds));
  const auto& stdout_output = output_strings[0];
  const auto& stderr_output = output_strings[1];

  // RDSS_VLOG_LINES(2, absl::StrCat(bin_name, " stdout:\n ", stdout_output));
  // RDSS_VLOG_LINES(2, absl::StrCat(bin_name, " stderr:\n ", stderr_output));

  // Wait for the subprocess to finish.
  ASSIGN_OR_RETURN(int exit_status, WaitForPid(pid));
  if (exit_status != 0) {
    return absl::InternalError(
