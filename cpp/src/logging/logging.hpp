...
//   RDSS_CHECK(!cheese.empty()) << "Out of Cheese";
//
...
// `RDSS_CHECK(foo)` and `RDSS_LOG_IF(INFO, !foo)` do nothing if `foo` is true.
// Even seemingly unconditional statements like `RDSS_LOG(INFO)` might be
// disabled at compile-time to minimize binary size or for security reasons.
//
// * Except for the condition in a `RDSS_CHECK` or `RDSS_QCHECK` statement,
//   programs must not rely on evaluation of expressions anywhere in logging
//   statements for correctness. For example, this is ok:
//
//     RDSS_CHECK((fp = fopen("config.ini", "r")) != nullptr);
//
//   But this is probably not ok:
//
//     RDSS_LOG(INFO)
//         << "Server status: " << StartServerAndReturnStatusString();
//
//   This is bad too; the `i++` in the `RDSS_LOG_IF` condition may not be
//   evaluated, which would make the loop infinite:
//
//     for (int i = 0; i < 1000000;) {
//         RDSS_LOG_IF(INFO, i++ % 1000 == 0) << "Still working...";
//     }
//
...
//   call into an `RDSS_LOG` statement.
...
//     RDSS_LOG(INFO) << "Number of cakes: " << num_of_cakes;  // Note the typo!
//
...
//       RDSS_LOG_IF(ERROR, false) << "The fox says " << fox_line1;
//       char fox_line2[] = "A-oo-oo-oo-ooo!";
//       RDSS_LOG(INFO) << "The fox also says " << fox_line2;
//     }
//
...
//
// Flag settings are not carried over from one `RDSS_LOG` statement to the next;
// this is a bit different than e.g. `std::cout`:
//
//   RDSS_LOG(INFO) << std::hex << 0xdeadbeef;  // logs "0xdeadbeef"
//   RDSS_LOG(INFO) << 0xdeadbeef;              // logs "3735928559"

#ifndef RDSS_LOGGING_LOGGING_H_
#define RDSS_LOGGING_LOGGING_H_

#include <absl/base/optimization.h>
#include <absl/status/status.h>

#include "check_ops.hpp"
...
// -----------------------------------------------------------------------------
//
// Most `RDSS_LOG` macros take a severity level argument. The severity levels
...
//
// Some preprocessor shenanigans are used to ensure that e.g. `RDSS_LOG(INFO)`
...
// `RDSS_LOG` macros evaluate to an unterminated statement. The value at the
// end of the statement supports some chainable methods:
//
...
//     `RDSS_VLOG(verbose_level)`. Unlike `RDSS_VLOG`, this method does not
...
// `RDSS_LOG` takes a single argument which is a severity level. Data streamed
...
//   RDSS_VLOG(2) << "I print when you run the program with --v=2 or higher";
//
...
#define RDSS_VLOG(verbose_level) RDSS_VLOG_IF(verbose_level, true)

// `RDSS_LOG_IF` and friends add a second argument which specifies a condition.
...
// -----------------------------------------------------------------------------
//
// `RDSS_CHECK` macros terminate the program with a fatal error if the specified
// condition is not true. Except for `RDSS_DCHECK`, they are not controlled by
// `NDEBUG` (cf. `assert`), so the check will be executed regardless of
// compilation mode. `RDSS_CHECK` and friends are thus useful for confirming
...
// `RDSS_CHECK` terminates the program with a fatal error if condition is not
// true. Example:
//
//   RDSS_CHECK(!cheese.empty()) << "Out of Cheese";
//
...
// otherwise. Unlike with `RDSS_CHECK`, it is not safe to rely on evaluation of
// `condition`.
#ifndef NDEBUG
...
// `RDSS_CHECK_EQ` and friends are syntactic sugar for `RDSS_CHECK(x == y)` that
// automatically output the expression being tested and the evaluated values on
// either side. Example:
//
//   int x = 3, y = 5;
//   RDSS_CHECK_EQ(2 * x, y) << "oops!";
//
// Might produce a message like:
//
...
// statement. Example:
//
//   RDSS_CHECK_EQ(std::string("abc")[1], 'b');
//
...
#ifndef NDEBUG
#define RDSS_DCHECK_EQ(val1, val2) RDSS_CHECK_EQ(val1, val2)
#define RDSS_DCHECK_NE(val1, val2) RDSS_CHECK_NE(val1, val2)
#define RDSS_DCHECK_LE(val1, val2) RDSS_CHECK_LE(val1, val2)
#define RDSS_DCHECK_LT(val1, val2) RDSS_CHECK_LT(val1, val2)
#define RDSS_DCHECK_GE(val1, val2) RDSS_CHECK_GE(val1, val2)
#define RDSS_DCHECK_GT(val1, val2) RDSS_CHECK_GT(val1, val2)
#else
#define RDSS_DCHECK_EQ(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#define RDSS_DCHECK_NE(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#define RDSS_DCHECK_LE(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#define RDSS_DCHECK_LT(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#define RDSS_DCHECK_GE(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#define RDSS_DCHECK_GT(val1, val2) RDSS_LOGGING_INTERNAL_DCHECK_NOP(val1, val2)
#endif
...
#define RDSS_CHECK_OK(val) RDSS_CHECK_EQ(::absl::OkStatus(), (val))
#define RDSS_QCHECK_OK(val) RDSS_QCHECK_EQ(::absl::OkStatus(), (val))
#define RDSS_DCHECK_OK(val) RDSS_DCHECK_EQ(::absl::OkStatus(), (val))

// `RDSS_DIE_IF_NULL` behaves as `RDSS_CHECK_NE` vs `nullptr` but also evaluates
// to its argument. It is useful in initializers where statements (like
// `RDSS_CHECK_NE`) can't be used. Outside initializers, prefer `RDSS_CHECK` or
// `RDSS_CHECK_NE`. `RDSS_DIE_IF_NULL` works for both raw pointers and
...
// to nullptr_t. For types that aren't raw pointers, `RDSS_DIE_IF_NULL` returns
